...
ECMAScript：定义JS的编程语法语法和基础核心知识，JavaScript负责实现（还有JSScript语言实现）。

DOM：文档对象模型。是一种标准编程接口，通过DOM提供的接口，可对页面上的各种元素进行操作（大小、颜色、位置等）。

BOM：浏览器对象模型。提供对浏览器窗口进行操作的接口（弹窗、控制浏览器跳转、获取分辨率等）。

![image-20211126100833866](前端学习第3章javascript.assets/202111261008006.png)

## JS的编写的位置  

1.可以编写到标签的指定属性中 ，虽然可以写在标签属性中，但是他们属于结构与行为耦合不推荐
...
JS底层保存标识符时实际上采用的是Unicode编码，所以理论上所有的utf-8中含有的字符都可以作为标识符。
...
十六进制：0x开头，例如 var a = 0xff

 八进制：0开头，例如 var a = “070”，parseInt(a)在部分浏览器中会返回70也可能返回56。

 可以在parseInt()中增加一个参数，来指定进制
...
```
 ...
 ```javascript
var obj = {}; 
//对象字面量属性名可以加引号也可以不加，若使用特殊名字必须加引号
var obj2 = {name:"wxz",age=18,"@@#!#!@":"瞎几把写"};
 ```
**向对象中添加属性**  
 语法：  
	对象.属性名 = 属性值;  
...
首先，JS的变量都保存在内存中，内存又分为栈内存和堆内存，基本数据类型的变量可直接保存在栈内存中，对象类型的变量保存在堆内存中，栈内存中只保留其引用。

所以当使用 “a = b” 为变量赋值时：

* 若 b 为基本数据类型，相当于在栈内存中为 a 复制栈内存中 b 的数据，赋完值后 b 再变化也不会影响到 a 的值。
* 若 b 为Object类型，b 在栈内存中的值是其堆内存中的引用地址，所以 a 复制的也是 b 的引用地址，此时 b 的值发生变化，a 通过引用地址指向的是 b 的值，也会一样变化。

![image-20211125161247821](前端学习第3章javascript.assets/202111251612941.png)
...
//例子
var fun1 = new Function("console.log('这是我第一个函数');");
fun1();
...
使用工厂模式创建的对象，构造函数使用的都是new Object，所以创建的对象都是Object类型，导致无法区分不同对象类型。
...
var p1 = createPerson("wxz",18,"male");
var p2 = createPerson("lmx",1,"female");
var dog = createDog("汪汪",2,"公")
p1.sayname();
p2.sayname();
dog.sayname();
```
...
[JS创建对象模式和优缺点详解](https://blog.csdn.net/victoryzn/article/details/77847024)  
...
![image-20211201095536600](前端学习第3章javascript.assets/202112010955733.png)
...
```javascript
//foreach(fun)函数需要传一个函数fun作为参数，该函数fun会被浏览器当作回调函数调用，数组长度多长就会调用几次。
//浏览器在调用fun函数时，会向fun传最多3个最少0个参数，依次为“值、索引、数组对象”。
array.forEach(function(element , index , obj){  
...
```html
//[0,100]的随机数
var a = Number.parseInt(Math.random()*91+10)
console.log(a);
```
...
![DOM HTML 树](前端学习第3章javascript.assets/202112031637550.gif)
...
这些BOM对象在浏览器中都是作为window对象的属性保存的，全局对象。  
...
**setInterval()**绑定给某个元素时，可以通过先调用clearInterval()方法清理掉已生成的定时器方式来避免同时生成多个不必要的定时器。
